...
# --- Kiểm thử nhanh module ---
if __name__ == "__main__":
    print("--- Chạy kiểm thử cho Module VQA Handler ---")
    try:
        # Tạo ảnh dummy
        dummy_image = Image.new("RGB", (100, 100), color="red")
        dummy_image_path = "/kaggle/working/dummy_image.png"
        dummy_image.save(dummy_image_path)
        print(f"--- Đã tạo ảnh giả tại: {dummy_image_path} ---")

        # Lấy API key (Kaggle → env)
        GOOGLE_API_KEY = None
        try:
        ...
        if not GOOGLE_API_KEY:
            raise RuntimeError("Không có GOOGLE_API_KEY")

        genai.configure(api_key=GOOGLE_API_KEY)
        gemini_model_for_test = genai.GenerativeModel("gemini-1.5-flash")

        vqa_handler = VQAHandler(model=gemini_model_for_test, rpm_limit=12, max_retries=2)
        test_question = "Bức ảnh này có màu chủ đạo là gì?"
        print(f"\n--- Gọi VQA: '{test_question}' ---")
        result = vqa_handler.get_answer(dummy_image_path, test_question)

        print("\n--- Kết quả VQA ---")
        print(f"Câu trả lời: {result['answer']}")
        print(f"Độ tự tin: {result['confidence']:.2f}")

        assert isinstance(result, dict)
        assert "answer" in result and "confidence" in result
        assert isinstance(result["confidence"], float)

        print("\n✅ Kiểm thử VQA Handler thành công!")
    except Exception as e:
        print(f"\n--- ❌ Bỏ qua kiểm thử VQA Handler: {e} ---")
